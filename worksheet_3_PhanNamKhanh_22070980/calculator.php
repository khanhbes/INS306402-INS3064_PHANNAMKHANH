...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_numeric($num1) || !is_numeric($num2)) {
        ...
        if ($operation === '/' && $n2 === 0.0) {
            $error = "Error: Không thể chia cho 0!";
        } else {
...
<head>
    ...
    <style>
        ...
        .calculator { padding: 20px; border: 1px solid #ccc; max-width: 350px; background: #f9f9f9; }
        .form-group { margin-bottom: 10px; }
        input[type="text"], select { width: 100%; padding: 8px; box-sizing: border-box; }
        button { width: 100%; padding: 10px; background: #28a745; color: white; border: none; cursor: pointer; }
        .result { margin-top: 15px; padding: 10px; background: #e2e3e5; border: 1px solid #d6d8db; }
        .error { margin-top: 15px; padding: 10px; background: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; }
    </style>
</head>
<body>
    ...
    <div class="calculator">
        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
            <div class="form-group">
                ...
                <input type="text" id="num1" name="num1" required value="<?php echo htmlspecialchars($_POST['num1'] ?? ''); ?>">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="num2" name="num2" required value="<?php echo htmlspecialchars($_POST['num2'] ?? ''); ?>">
            </div>
